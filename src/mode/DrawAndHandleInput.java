...
public class DrawAndHandleInput implements GLEventListener, KeyListener, MouseListener
{
	 ...
	 */
	private Point[] vTransformedList;
...
//			vTransformedList = new Point[ReadObjectAndViewingFiles.v.length];
...
//				for(int vIdx : p.PointIndices)
//				{
//					Point v = ReadObjectAndViewingFiles.v[vIdx];
//					Point vTransformed = nPer.multiply(v); // apply transformation
//					if(rotationDoneSoFar != null) vTransformed = rotationDoneSoFar.multiply(vTransformed);
//
//					// take three, also save the transformed Point
...
//
//						/* 
//						 * Save the transformed Point in 3D so we can use it without
//						 * the need to recompute the values.
//						 */					
//						vTransformedList[vIdx] = vTransformed;
//
//						Point v2D = tranformPointTo2D( vTransformed ); // transformed to 2D
//						gl.glPoint2d( v2D.getX1(), v2D.getX2() );
//					}
//					gl.glEnd();
//				}
//			}
...
//					Point vTransformed = vTransformedList[vIdx];
//					Point v2D = tranformPointTo2D( vTransformed );
//					gl.glPoint2d( v2D.getX1(), v2D.getX2() );
//				}
//				gl.glEnd();
//			}
//		}
	...
	private void revertAllChanges() {
		...
		ReadObjectAndViewingFiles.vrp = new Point(
		...
		ReadObjectAndViewingFiles.prp = new Point(
	 ...
	 * @return
	 */
	private Point tranformPointTo2D(Point vTransformed) {
	 ...
	 * @return
	 */
	private boolean checkBackFace(ArrayList<Point> testVertices) {
		/**
	...
	private static Matrix normalizingToCvv() {
		 ...
		 */
		Matrix translateVRP = getMatrixTranslatePointToOrigin(ReadObjectAndViewingFiles.vrp);
		Matrix rotateVRC = getMatrixRotateVRC();
		Matrix translatePRP = getMatrixTranslatePointToOrigin(ReadObjectAndViewingFiles.prp);
	...
	/**
	 * General method for translating a Point to origin
	 * @param x
	 * @return
	 */
	private static Matrix getMatrixTranslatePointToOrigin(Point x) {
		double[][] data = {
				{1,0,0, -1 * x.getX1()},
				{0,1,0, -1 * x.getX2()},
				{0,0,1, -1 * x.getX3()},
